...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    x = torch.tensor(list, dtype=torch.float64)
    print(x)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    #     result_loop = torch.mm(x[i], y[i])
    #     z.append(result_loop)
    for a, b in zip(x, y):
        result_loop = torch.mm(a, b)
        z.append(result_loop)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    sum_col = x.sum(dim=0)
    mean_col = sum_col / x.shape[0]
    std_col = (((x - mean_col) ** 2).sum(dim=0) / (x.shape[0] - 1)).sqrt()
    y = (x - mean_col) / std_col
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
